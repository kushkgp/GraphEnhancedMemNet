...
from model_3layer import MemN2N
...
flags.DEFINE_integer("nhop", 4, "number of hops [7]")
...
def main(_):
	...
	'''
	mats_pair_list = train_data[5]
	mats_list = []
	for mats_pair in mats_pair_list:
		...
		for i,_ in enumerate(Ws):
			...
			W_rMm.append(W_Mm)
		mats_list.append(W_rMm)
	train_data = list(train_data)
	train_data[5] = np.array(mats_list)
	train_data = tuple(train_data)
	pkl.dump(train_data, open('train_data_laptop_clean.pkl', 'w'))
	train_data = 0
	return

	mats_pair_list = test_data[5]
	mats_list = []
	for mats_pair in mats_pair_list:
		Ws, Ms = mats_pair[0], mats_pair[1]
		W_rMm = []
	...
	test_data[5] = np.array(mats_list)
	test_data = tuple(test_data)

	pkl.dump(test_data, open('test_data_laptop_clean.pkl', 'w'))
	'''
	...
	# GraphMemNetData = pkl.load(open('TOTAL_restaurant_init.pkl','r'))
	GraphMemNetData = pkl.load(open('TOTAL_laptop_init.pkl','r'))
	...
	Wmm_train = GraphMemNetData[0][5]
	Wmm_test = GraphMemNetData[1][5]
	Wrm = train_data[5]
	for index, wma in enumerate(Wma_train):
		wam = np.reshape(wma,(1,-1))
		a = np.concatenate((Wrm[index][0], wam),axis=0)
		Wrm[index][1].append(Wmm_train[index])
		b = copy.deepcopy(Wrm[index][1])
		Wrm[index] = (a, b)
	pkl.dump(train_data, open('temp.pkl','w'))
	Wrm = test_data[5]
	for index, wma in enumerate(Wma_test):
		wam = np.reshape(wma,(1,-1))
		a = np.concatenate((Wrm[index][0], wam),axis=0)
		Wrm[index][1].append(Wmm_test[index])
		b = copy.deepcopy(Wrm[index][1])
		Wrm[index] = (a, b)
	...
	print "Dump Loaded!!!"
	print len(test_data[5]), len(test_data1[0]), len(test_data2[0])
