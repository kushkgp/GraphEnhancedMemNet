...
class MemN2N(object):
    ...
    def build_memory(self):
      ...
      self.Fout3dim = tf.matmul(self.Att3dim, self.R) #batch-size * 1 * edim
      self.Fout2dim = tf.reshape(self.Fout3dim, [self.batch_size, -1]) #batch_size * edim
      self.hid.append(self.Fout2dim)

    def build_model(self):
      self.build_memory()

      self.W = tf.Variable(tf.random_uniform([self.edim, 3], minval=-0.01, maxval=0.01)) #edim * 3
