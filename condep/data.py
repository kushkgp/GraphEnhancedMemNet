...
def get_dataset(data_file_name, sent_word2idx, target_word2idx, embeddings):
  ...
  with open(data_file_name, 'r') as data_file:
    ...
    for line_no in range(0, len(lines)-1, 3):
      ...
      try:
        # DI_mm, W_ma = np.zeros([70,70]), np.zeros([70,1])
        # DI_mm, W_ma = Ctree.getReuiredParameters(sentence = original_sentence, aspect_words_indexes = target_locations)
        W_rm = Ctree.getRequiredParameters(sentence = original_sentence, aspect_words_indexes = target_locations)
        # rules_freq = rules_freq + np.sum(W_rm, axis = 1)
      ...
      W_ma_list.append(W_rm)
  # print "Rules Frequency - ", rules_freq
  return sentence_list, location_list, target_list, polarity_list, original_sentence_list, W_ma_list
